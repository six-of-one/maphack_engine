...
// load into heap
	...
	for (i = 0;i < mod->brush.numsubmodels;i++)
	{
		...
		if (i > 0)
		{
			...
			dpsnprintf(name, sizeof(name), "*%i", i);  // this is where the submodel *{n} is done !  we think so anyway
...
//				- pick the first free slot, but if i = 1, skip one (allocate it!) because the map ent takes taht slot
//				- set a precache search string to match the index - 1...quake-c MUST set for ents desiring to use this submodel - modelindex, model
		if (mod->brush.submodels[i]->name[0] == '*' && sv.worldmodel)
		{
			int m_submodels;
			m_submodels = 0;
			for (k = 1;k < MAX_MODELS;k++)
			{
				if (!sv.models[k])
				{
					m_submodels = k;
			...
			if (i == 1) sv.models[m_submodels++] = mod; // need to bump ahead one because the map ent will occupy this slot, but leave it indic. used
			if (sv.models[m_submodels]) Con_Printf("* XXX load - sv.models[ %i ] occupied - %s\n", m_submodels, sv.models[m_submodels]->name);
			sv.models[m_submodels] = mod->brush.submodels[i];
			dpsnprintf(sv.model_precache[m_submodels], sizeof(sv.model_precache[m_submodels]), "*%i", m_submodels - 1);
