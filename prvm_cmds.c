...
/*
=================
VM_ent_load						(Cataboligne - 015.7.10)
																			- 015.12.6 - expand to handle bsp files and map brush models
																			- 015.12.8 - move submodels into sv.models[] array, set cvar for index
																			- 015.12.13 - added self protect
...
static void ent_load(const char *fname)
{
	...
	sv_self = PRVM_clientglobaledict(self);
	ps = strlen(fname) - 3;

// silly, but I cant seem to find a better way
// the coda of the hacker / coder - it works...fuck it
	if (fname[ps] == 'b' && fname[ps+1] == 's' && fname[ps+2] == 'p')
	{
		entmodel = Mod_ForName(fname, false, developer.integer > 0, NULL);
		if (!entmodel || !entmodel->TraceBox)
		{
			Con_Printf("* Couldn't load map chunk %s\n", fname);
			return;
		}
		PRVM_ED_LoadFromFile (entmodel->brush.entities);
		Con_DPrintf("Loaded entities from map ( %s )\n", fname);
	}
	else
	{
		if ((entities = (char *)FS_LoadFile(va("%s", fname), tempmempool, true, NULL)))
		{
			Con_DPrintf("Loaded entities from ( %s )\n", fname);
			PRVM_ED_LoadFromFile (entities);
			Mem_Free(entities);
		}
	}
	PRVM_clientglobaledict(self) = sv_self;
}
