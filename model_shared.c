...
// Cataboligne - 015.12.5 - ent_load extension for map_hack - load *.bsp and *.ent file entites - if developer >= 2 display sv.model array contents
	...
	if (developer.integer > 1)
	{
		...
		for (i = 1; i+1 < MAX_MODELS;i++)
		{
			if (sv.models[i])
				Con_Printf("%4iK %6i crc - ix:[ %3i ]  pre: %4s  %s\n", sv.models[i]->mempool ? (int)((sv.models[i]->mempool->totalsize + 1023) / 1024) : 0, sv.models[i]->crc, i, sv.model_precache[i], sv.models[i]->name);
//			else
