...
// Cataboligne - 8.7.14 - always render a sky - map_hack
cvar_t r_sky_always = {CVAR_SAVE, "r_sky_always", "1", "enables sky rendering always - even for one hidden little bit of sky - for map hack system"};
...
// Cataboligne - 8.7.14 - always render a sky if there is even one little bit anywhwere on a map - for map hack system

	if (r_sky_always.integer)
	{
		skyrenderbox = true;
		skyrendermasked = true;
	}
...
void R_Sky_Init(void)
{
	...
	Cvar_RegisterVariable (&r_sky_always);
